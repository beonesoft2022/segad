				...
				<div style="overflow: hidden !important;display: flex; flex-wrap: wrap;align-content: center;width: {{$barcode_details->width * 1}}in; height: {{$barcode_details->height * 1}}in; justify-content: center;">
					...
					<div>
						...
						@if(!empty($print['business_name']))
							<b style="font-weight: 900 !important;display: block !important; font-size: {{$print['business_name_size']}}px">{{$business_name}}</b>
						@endif
	
						{{-- Product Name --}}
						@if(!empty($print['name']))
							<span style="font-weight: 900 !important;display: block !important; font-size: {{$print['name_size']}}px">
						...
						@if(!empty($print['variations']) && $page_product->is_dummy != 1)
							<span style="font-weight: 900 !important;display: block !important; font-size: {{$print['variations_size']}}px">
						...
						<span style="font-weight: 900 !important;font-size: {{$print['price_size']}}px;">
							Price :
						...
						{{-- Barcode --}}
						<img style="width:110% !important;height: {{$barcode_details->height*0.24}}in !important; display: block;" src="data:image/png;base64,{{DNS1D::getBarcodePNG($page_product->sub_sku, $page_product->barcode_type, 1,30, array(0, 0, 0), false)}}">
						
						<span style="font-weight: 900 !important;font-size: 12px !important">
