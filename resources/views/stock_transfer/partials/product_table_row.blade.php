<tr class="product_row">
    ...
    @if( session()->get('business.enable_lot_number') == 1 || session()->get('business.enable_product_expiry') == 1)
      ...
      @if($product->enable_stock == 1)
        ...
        <small class="text-muted" style="white-space: nowrap;">@lang('report.current_stock'): <span
                  class="qty_available_text">{{$product->formatted_qty_available}}</span> {{ $product->unit }}</small>
      @endif
  ...
  <td>
    ...
    <input type="text" class="form-control product_quantity input_number input_quantity"
           ...
           @if($product->unit_allow_decimal == 1) data-decimal=1 @else data-rule-abs_digit="true"
           data-msg-abs_digit="@lang('lang_v1.decimal_value_not_allowed')" data-decimal=0 @endif
  ...
  <td class="@if (!auth()->user()->can('transfer.canviewprice')) hide @endif">
    <input type="text" name="products[{{$row_index}}][unit_price]" class="form-control product_unit_price input_number"
           value="{{@num_format($product->last_purchased_price * $multiplier)}}">
  </td>
  <td>
    <input type="text" readonly name="products[{{$row_index}}][price]" class="form-control product_line_total"
